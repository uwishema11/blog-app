...
export const createComment = catchAsync(async (req, res) => {
  ...
  return successResponse(res, 201, 'Comment successfully created', newComment);
});
...
export const deleteComment = catchAsync(async (req, res, next) => {
  ...
  if (result === 0) {
    return handleError(res, 404, 'Comment not found or does not belong to the specified blog');
  }
...
export const findAllComments = catchAsync(async (req, res, next) => {
  ...
  return successResponse(res, 200, 'Fetched data', data);
});
...
export const updateComment = catchAsync(async (req, res) => {
  ...
  if (!comment) {
     return handleError(res, 404, 'comment not not exits! check if it not deleted');
  }
  await Services.updateComment(req.params.id, req.body);

  return successResponse(res, 201, 'comment updated successfully');
});
